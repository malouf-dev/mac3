...
# mac3 install or your Steam copy of the original GTA III, and drops the
# finished, ready-to-play mac3.app into your Downloads folder. Drag it into
...
#   --dmg            also wrap the app in a drag-to-Applications mac3.dmg
...
#                    3.app", a Wineskin/Wine wrapper, or an installed
#                    mac3.app). If given, it is used instead of an existing
#                    mac3 or Steam install. With no path, an installed
#                    mac3.app, the Steam install, and then ~/Downloads (entries
...
# upgrades don't need the original install around anymore. re3.app is the name
# this app shipped under before it was renamed; people upgrading from it still
# have their game files in there, so look for both.
EXISTING_SRC=""
for app in "/Applications/mac3.app" "$HOME/Applications/mac3.app" "$DL/mac3.app" \
...
# wrapper (e.g. "GTA III Mac.app"), or a mac3.app. The ".app" suffix
# may be left off the path.
...
step "Baking the game files into mac3.app"
...
if [ "$FANCY" = yes ]; then
	...
	echo "==> Baking the game files into mac3.app (about 1.2 GB)..."
	bake_files
fi
...
if [ "$WANT_DMG" = no ]; then
	...
	rm -rf "$DL/mac3.app"
	mv "$APP" "$DL/mac3.app"
	ok "Placed mac3.app in $DL"
	outro "$DL/mac3.app is ready." \
		"Drag it into your Applications folder (or just double-click it) to play."
	open -R "$DL/mac3.app"
	exit 0
fi

step "Building mac3.dmg"
...
tell application "Finder"
  tell disk "$VOLNAME"
    ...
    set position of item "mac3.app" of container window to {130, 165}
...
rm -f "$DL/mac3.dmg"
dmg_convert() { hdiutil convert "$TMPDMG" -format UDZO -imagekey zlib-level=9 -o "$DL/mac3.dmg" >/dev/null; }
run_task "Compressing mac3.dmg" dmg_convert || die "hdiutil convert failed"
ok "Created $DL/mac3.dmg"

outro "$DL/mac3.dmg is ready." \
	"Opening it now — drag mac3 into the Applications folder to install."
open "$DL/mac3.dmg"
